...
public class FeedIteratorTest {
    ...
    public void can_read_empty_feed_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint, new FeedTransactor.NoOp(), null, null,
    ...
    public void can_read_single_page_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint.add("foo"), new FeedTransactor.NoOp(), null, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(0);
    ...
    public void can_read_single_page_with_location_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint.add("foo", "bar"), new FeedTransactor.NoOp(), 0, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(1);
        assertThat(pages).hasSize(1);
    ...
    public void can_read_single_page_with_boundary_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(endpoint.add("foo", "bar"), new FeedTransactor.NoOp(), null, 1, (page, origin) -> {
    ...
    public void can_read_single_page_complete_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint.add("foo", "bar"), new FeedTransactor.NoOp(), null, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(1);
        assertThat(pages).hasSize(1);
    ...
    public void can_read_multiple_page_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint.add("foo", "bar", "qux"), new FeedTransactor.NoOp(), null, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(2);
    ...
    public void can_skip_page_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(
            endpoint.add("foo", "bar", "qux", "baz"), new FeedTransactor.NoOp(), 2, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(3);
        assertThat(pages).hasSize(1);
    ...
    public void can_abort_reading_forward() throws InterruptedException {
        assertThat(FeedIterator.FORWARD.read(endpoint.add("foo", "bar", "qux"), new FeedTransactor.NoOp(), null, null, (page, origin) -> {
            pages.add(page);
            return false;
        }, () -> true)).contains(2);
        assertThat(pages).hasSize(1);
    ...
    public void can_read_single_page_with_boundary_backward() throws InterruptedException {
        assertThat(FeedIterator.BACKWARD.read(endpoint.add("foo", "bar"), new FeedTransactor.NoOp(), null, 1, (page, origin) -> {
            throw new AssertionError();
        }, () -> true)).contains(1);
    }
    ...
    public void can_skip_page_backward() throws InterruptedException {
        assertThat(FeedIterator.BACKWARD.read(
            endpoint.add("foo", "bar", "qux"), new FeedTransactor.NoOp(), 2, null,
            (page, origin) -> pages.add(page),
            () -> true
        )).contains(2);
        assertThat(pages).hasSize(2);
    ...
    public void can_abort_reading_backward() throws InterruptedException {
        assertThat(FeedIterator.BACKWARD.read(endpoint.add("foo", "bar", "qux"), new FeedTransactor.NoOp(), null, null, (page, origin) -> {
            pages.add(page);
            return false;
        }, () -> true)).contains(2);
        assertThat(pages).hasSize(1);
